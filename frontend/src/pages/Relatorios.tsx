...
const reportTypes: ReportType[] = [
  ...
  { value: "kpi", label: "KPI" },
  { value: "hsm", label: "HSM" },
  { value: "status_linha", label: "Status de Linha", digitalExcluded: true },
  { value: "adm_linhas", label: "Adm Linhas", adminOnly: true },
  { value: "envios", label: "Envios" },
  { value: "indicadores", label: "Indicadores" },
  { value: "tempos", label: "Tempos" },
  { value: "templates", label: "Templates", digitalExcluded: true },
  { value: "completo_csv", label: "Completo CSV" },
  { value: "equipe", label: "Equipe" },
  { value: "dados_transacionados", label: "Dados Transacionados", digitalExcluded: true },
  { value: "detalhado_conversas", label: "Detalhado Conversas" },
  { value: "linhas", label: "Linhas", digitalExcluded: true },
  { value: "mensagens_por_linha", label: "Mensagens por Linha", digitalExcluded: true },
  ...
  {
    value: "ativadores",
    label: "Ativadores (Linhas Criadas)",
    adminOnly: true,
  },
];
...
export default function Relatorios() {
  ...
  const filteredReportTypes = useMemo(() => {
    return reportTypes.filter((type) => {
      ...
      if (type.digitalExcluded && user?.role === "digital") {
        return false;
      }
      return true;
    });
  ...
  const handleGenerate = async () => {
    ...
    try {
      const blob = await reportsService.generate({
        ...
        type: reportType,
        onlyMovimentedLines:
          reportType === "linhas" ? onlyMovimentedLines : undefined,
      });
          ...
          <GlassCard>
            ...
            {reportType === "linhas" && (
              <div className="mt-4 flex items-center space-x-2">
                ...
                />
                <Label
                  htmlFor="onlyMovimentedLines"
                  className="text-sm font-normal cursor-pointer"
                >
                  Apenas linhas movimentadas (com conversas/campanhas no
                  período)
                </Label>
              </div>
            )}
              ...
              <div className="p-6">
                <div className="flex flex-col items-center justify-center py-12">
                  ...
                  <p className="text-sm text-muted-foreground mb-6 text-center">
                    Relatório:{" "}
                    <strong>
                      {lastGeneratedReportLabel || getSelectedReportLabel()}
                    </strong>
                    <br />
