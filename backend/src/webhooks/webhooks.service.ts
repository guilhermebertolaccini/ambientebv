    ...
    try {
      ...
      if (data.event === 'messages.upsert' || data.event === 'MESSAGES_UPSERT') {
        ...
        if (!isGroup) {
          ...
          // --- INTEGRAÇÃO API WHATSAPP CPC (INBOUND) ---
          // Se o contato tiver um CPF configurado com exatos 3 dígitos, e a mensagem do usuário bater exatamente com esses 3 dígitos, consideramos que ele "confirmou"
          if (contact.cpf && contact.cpf.length === 3 && contact.contract) {
            const cleanMessageTxt = messageText.trim();
            if (cleanMessageTxt === contact.cpf) {
              console.log(`✅ [Webhook CPC] Cliente confirmou os 3 dígitos do CPF (${contact.cpf}). Registrando acionamento...`);

              let segmentName = 'Default';
              if (line.segment) {
                const segmentObj = await this.prisma.segment.findUnique({ where: { id: line.segment } });
                if (segmentObj) segmentName = segmentObj.name;
              }

              const cpcRegistered = await this.cpcService.registerAcionamento(contactIdentifier, contact.contract, segmentName);
              if (cpcRegistered) {
                console.log(`✅ [Webhook CPC] Acionamento registrado com sucesso na API CPC. (Tel: ${contactIdentifier}, Contrato: ${contact.contract})`);
              } else {
                console.warn(`⚠️ [Webhook CPC] Falha ao registrar acionamento na API CPC.`);
              }
            }
          }
          // ---------------------------------------------
